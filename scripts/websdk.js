 ...
 *
 * Config is author-controlled via the site's DA metadata sheet, so no code
 * change is needed to wire a real Adobe org:
 ...
 * SYNCHRONOUSLY (so any consumer that fires before the library finishes
 * loading still finds a callable `window.alloy`), configures it, then loads
 * the library async to drain the queue. Call from the site's lazy-phase hook
 * (scripts/lazy.js) before sections decorate.
 */
