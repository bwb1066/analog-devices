...
const SITE_KEY = getMetadata('commerce-site') || 'analog-devices';
const CATALOG_URL = getMetadata('commerce-catalog') || '/drafts/data/analog-devices-catalog.json';
// LOCAL by default so the scraped static catalog is fully demoable with no
// backend. A page opts into the shared Supabase backend (REMOTE) by authoring
// a `commerce-url` metadata row — analog.com's DA metadata currently does, so
// remove that row to serve the local catalog, or seed brand_products to use it.
const COMMERCE_URL = getMetadata('commerce-url') || '';
// eslint-disable-next-line max-len
const COMMERCE_KEY = getMetadata('commerce-key') || getMetadata('concierge-key') || DEFAULT_KEY;
